...
- When the user supplies an executive HTML reference and asks for it as-is, treat that HTML as the fidelity source for layout, assets, motion, interactions, and responsive behavior; do not reinterpret or modernize it without explicit approval.
- The approved 2026-08-12 Services, Portfolio, and Offers HTML uploads are the current structural/motion fidelity sources for `/services`, `/portfolio`, and `/offers`, while the live SWEED Helvetica Arabic typography and `#261b3e` / `#ed2062` brand palette are intentionally bridged over them at runtime.
