...
TASKS_DIR="/media/amr/02D804FAD804EE2B/Busniss/SWEED-Website/.opencode/tasks"
if [ -d "$TASKS_DIR" ]; then
  STALE_COUNT=$( (find "$TASKS_DIR" -maxdepth 1 -type d -mtime +1 2>/dev/null || true) | wc -l )
  if [ "$STALE_COUNT" -gt 0 ]; then
    echo "⚠️  Found $STALE_COUNT stale task directories in /media/amr/02D804FAD804EE2B/Busniss/SWEED-Website/.opencode/tasks/"
    echo "   These are leftover from crashed subagent sessions."
    echo "   Run: rm -rf /media/amr/02D804FAD804EE2B/Busniss/SWEED-Website/.opencode/tasks/*  (safe — only affects dead sessions)"
  fi
fi
```
