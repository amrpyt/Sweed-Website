---
name: i-convex-create-component
description:
...
7. Wire the component into the app with `app.use(...)`. If the app does not
   already have `convex/i-convex.config.ts`, create it.
...
- Install it from the app's `convex/i-convex.config.ts` with `app.use(...)`
...
```ts
// convex/components/notifications/i-convex.config.ts
...
```ts
// convex/i-convex.config.ts
import { defineApp } from "convex/server";
import notifications from "./components/notifications/i-convex.config.js";
